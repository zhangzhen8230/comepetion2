...
def create_pretrained_embedding(pretrained_weights_path, trainable=False, **kwargs):
    ...
    in_dim, \
    out_dim = pretrained_weights.shape
...
def train():
    ...
    for k,(tr,va) in enumerate(StratifiedKFold(Train_label,random_state=27,n_folds=N_FOLD)):
        ...
        print ("Train...")
        checkpoint = ModelCheckpoint('./model_file/attention_{}.hdf5'.format(k), monitor='val_loss', verbose=1,
...
def test():
    ...
    for i in range(N_FOLD):
        model.load_weights('./model_file/attention_{}.hdf5'.format(i))
